...
<div class="row price-group">
    ...
    <div class="col-md-4">
        <x-core::form.text-input
            ...
            name="sku"
            :value="old('sku', $product ? $product->sku : (new HuberwayCMS\Ecommerce\Models\Product())->generateSku())"
        />
...
<x-core::form.fieldset class="stock-status-wrapper" @style(['display: none' => old('with_storehouse_management', $product ? $product->with_storehouse_management : $originalProduct->with_storehouse_management ?? 0) == 1])>
    ...
    @foreach (HuberwayCMS\Ecommerce\Enums\StockStatusEnum::labels() as $status => $label)
        <x-core::form.checkbox
            :label="$label"
            name="stock_status"
            type="radio"
...
@if (
    ...
    || (!$product && !$originalProduct &&  request()->input('product_type') != HuberwayCMS\Ecommerce\Enums\ProductTypeEnum::DIGITAL)
