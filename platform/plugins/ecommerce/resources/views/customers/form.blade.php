...
@section('form_end')
    ...
    >
        ...
        @include('plugins/ecommerce::customers.addresses.form', [
            'address' => new HuberwayCMS\Ecommerce\Models\Address(),
        ])
    ...
    @if ($customerId = $form->getModel()->id)
        <div class="customer-reviews-table widget meta-boxes">
            <x-core::card>
                ...
                <div>
                    {!! app(HuberwayCMS\Ecommerce\Tables\CustomerReviewTable::class)->customerId($customerId)->setAjaxUrl(route('customers.ajax.reviews', $customerId))->renderTable() !!}
                </div>
            </x-core::card>
        </div>
    @endif
