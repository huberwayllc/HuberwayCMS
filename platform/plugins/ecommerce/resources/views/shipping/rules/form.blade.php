    ...
    <x-core::form-group>
        ...
        {!! Form::customSelect(
            ...
            ['' => trans('plugins/ecommerce::shipping.rule.select_type')] +
                HuberwayCMS\Ecommerce\Enums\ShippingRuleTypeEnum::availableLabels($rule ? $rule->shipping : null),
            $rule ? $rule->type : '',
            ['class' => 'select-rule-type'],
            HuberwayCMS\Ecommerce\Enums\ShippingRuleTypeEnum::toSelectAttributes(),
        ) !!}
    </x-core::form-group>
    ...
    <div class="rule-from-to-inputs" @style(['display: none' => $rule && !$rule->type->showFromToInputs()])>
        <x-core::form-group>
            <x-core::form.label class="rule-from-to-label">
                {{ $rule ? $rule->type->label() : HuberwayCMS\Ecommerce\Enums\ShippingRuleTypeEnum::BASED_ON_PRICE()->label() }}
            </x-core::form.label>
                ...
                <div class="w-full">
                    <div class="input-group input-group-flat">
                        <span class="input-group-text unit-item-label">
                            {{ $rule ? $rule->type->toUnit() : HuberwayCMS\Ecommerce\Enums\ShippingRuleTypeEnum::BASED_ON_PRICE()->toUnit() }}
                        </span>
