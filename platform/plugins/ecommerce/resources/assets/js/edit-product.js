class EcommerceProduct {
    ...
    handleEvents() {
        ...
        $(document).on('click', '.btn-trigger-delete-selected-variations', (event) => {
            ...
            if (ids.length === 0) {
                HuberwayCMS.showError(HuberwayCMSVariables.languages.tables.please_select_record)
                return false
            }
    ...
    handleAddVariations() {
        ...
        let createOrUpdateVariation = ($current) => {
            ...
            formData = HuberwayCMS.unmaskInputNumber($form, formData)
...
$(() => {
    ...
    $(document).on('click', '[data-bb-toggle="product-search-advanced"]', (event) => {
        ...
        if ($formBody.find('.card-body').length === 0) {
            HuberwayCMS.showLoading($formBody)
            ...
            $.ajax({
                ...
                complete: () => {
                    HuberwayCMS.hideLoading($formBody)
                }
            })
        }
    })
    ...
    let loadRelationBoxes = () => {
        ...
        if ($wrapBody.length) {
            HuberwayCMS.showLoading($wrapBody)
            ...
            $.ajax({
                ...
                complete: () => {
                    HuberwayCMS.hideLoading($wrapBody)
                }
            })
        }
    }
    ...
    function humanFileSize(bytes, si = true, dp = 2) {
        ...
        return HuberwayCMS.numberFormat(bytes, dp, ',', '.') + ' ' + units[u]
    }
