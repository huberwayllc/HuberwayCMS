    ...
    init() {
        ...
        $(document).on('keyup', '.input-sync-item', (event) => {
            ...
            $this
                ...
                .text(HuberwayCMS.numberFormat(parseFloat(number), 2))
        })
        ...
        $(document).on('click', '#save-shipping-rule-item-detail-button', (event) => {
            ...
            if (! allowedMethods.includes(method)) {
                HuberwayCMS.showError('This method is not supported.')
        ...
        HuberwayCMS.select($(document).find('.select-country-search'))
        ...
        $(document).on('keyup', '.base-price-rule-item', (event) => {
            ...
            $.each($(document).find('.support-shipping .rule-adjustment-price-item'), (index, item) => {
                ...
                $(item).text(HuberwayCMS.numberFormat(parseFloat(basePrice) + parseFloat(adjustmentPrice)), 2)
            })
        })
