...
<head>
    ...
    <link
        href="{{ asset('vendor/core/core/base/images/icona-huberway-colore.png') }}"
        rel="icon"
    >
            ...
            <div class="container-xl installer-container">
                <div class="row mb-6">
                    <div class="col" style="display:flex;">
                        <img src="https://huberway.com/assets/images/common/logo_icon_white.svg" style="height: 41px;margin-left: auto;margin-right: 11px;">
                        <h2 style="margin-right:auto" class="h1 page-title justify-content-center text-white">
        ...
        @if (Session::has('success_msg') || Session::has('error_msg') || (isset($errors) && $errors->any()) || isset($error_msg))
            <script type="text/javascript">
                $(function() {
                    ...
                    HuberwayCMS.showSuccess('{{ session('success_msg') }}');
                    @endif
                    @if (Session::has('error_msg'))
                    HuberwayCMS.showError('{{ session('error_msg') }}');
                    @endif
                    @if (isset($error_msg))
                    HuberwayCMS.showError('{{ $error_msg }}');
                    @endif
                    @if (isset($errors))
                    @foreach ($errors->all() as $error)
                    HuberwayCMS.showError('{{ $error }}');
                    @endforeach
                    @endif
                })
            </script>
