...
class HuberwayCMS {
    ...
    constructor() {
        ...
        HuberwayCMS.initResources()
        HuberwayCMS.initGlobalResources()
        HuberwayCMS.handleCounterUp()
        HuberwayCMS.initMediaIntegrate()

        if (HuberwayCMSVariables && HuberwayCMSVariables.authorized === '0' && typeof route !== 'undefined') {
    ...
    static blockUI(options) {
        ...
        HuberwayCMS.showLoading(options.target)
    }

    static unblockUI(target) {
        HuberwayCMS.hideLoading(target)
    }
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (HuberwayCMS.noticesTimeout[key]) {
            clearTimeout(HuberwayCMS.noticesTimeout[key])
        }

        HuberwayCMS.noticesTimeout[key] = setTimeout(() => {
            if (!messageHeader) {
                switch (messageType) {
                    case 'error':
                        messageHeader = HuberwayCMSVariables.languages.notices_msg.error
                        break
                    case 'success':
                        messageHeader = HuberwayCMSVariables.languages.notices_msg.success
                        break
                }
            }
        ...
        }, HuberwayCMS.noticesTimeoutCount)
    }
    ...
    static handleError(data) {
        if (typeof data.errors !== 'undefined' && !_.isArray(data.errors)) {
            HuberwayCMS.handleValidationError(data.errors)
        } else {
            if (typeof data.responseJSON !== 'undefined') {
                if (typeof data.responseJSON.errors !== 'undefined') {
                    if (data.status === 422) {
                        HuberwayCMS.handleValidationError(data.responseJSON.errors)
                    }
                } else if (typeof data.responseJSON.message !== 'undefined') {
                    HuberwayCMS.showError(data.responseJSON.message)
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            HuberwayCMS.showError(item)
                        })
                    })
                }
            } else {
                HuberwayCMS.showError(data.statusText)
            }
        }
    }
    ...
    static async copyToClipboard(textToCopy) {
        if (navigator.clipboard && window.isSecureContext) {
            ...
            HuberwayCMS.unsecuredCopyToClipboard(textToCopy)
        }
    }
    ...
    static initResources() {
        ...
        $.each($(document).find('.select-search-ajax'), function (index, element) {
            if ($(element).data('url')) {
                ...
                HuberwayCMS.select(element, options)
            }
        })
        ...
        HuberwayCMS.initDatePicker('.datepicker')

        if (jQuery().textareaAutoSize) {
            $('textarea.textarea-auto-height').textareaAutoSize()
        }

        HuberwayCMS.initCodeEditorComponent()
        HuberwayCMS.initColorPicker()
        HuberwayCMS.initLightbox()
        HuberwayCMS.initTreeCategoriesSelect()
        HuberwayCMS.initCoreIcon()
    ...
    static initGlobalResources() {
        ...
        HuberwayCMS.initFieldCollapse()
        HuberwayCMS.initTreeCheckboxes()
        HuberwayCMS.initClipboard()
        HuberwayCMS.initDropdownCheckboxes()
    }
    ...
    handleTurnOffDebugMode() {
        ...
        $submit.on('click', function (event) {
            ...
            HuberwayCMS.showButtonLoading($submit[0])
            ...
            $httpClient
                ...
                .finally(() => {
                    HuberwayCMS.hideButtonLoading($submit[0])
                })
        })
    }
    ...
    static initMediaIntegrate() {
        if (jQuery().rvMedia) {
            HuberwayCMS.gallerySelectImageTemplate = `
            ...
            const gallerySelectImages = function (files, $currentBoxList, excludeIndexes = []) {
                let template = HuberwayCMS.gallerySelectImageTemplate
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown((event) => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', HuberwayCMS.onDragTool)
            $(window).on('mouseup', HuberwayCMS.onReleaseTool)
        })
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', HuberwayCMS.onDragTool)
        $(window).off('mouseup', HuberwayCMS.onReleaseTool)
    }
    ...
    static initFieldCollapse() {
        $(document).on('click, change', '[data-bb-toggle="collapse"]', function (e) {
            ...
            switch (e.currentTarget.type) {
                ...
                default:
                    console.warn(`[HuberwayCMS] Unknown type ${e.currentTarget.type} of collapse`)
    ...
    static initCodeEditorComponent() {
        $(document)
            ...
            .each(function () {
                HuberwayCMS.initCodeEditor(this, this.dataset.mode || 'htmlmixed')
            })
    }
    ...
    static initLightbox() {
        ...
        a.forEach((element) => {
            ...
            if (!instance[instanceName]) {
                instance[instanceName] = HuberwayCMS.lightbox()
            }
    ...
    static initClipboard() {
        $(document).on('click', '[data-bb-toggle="clipboard"]', async (e) => {
            ...
            await HuberwayCMS.copyToClipboard(text)

            if (copiedMessage) {
                HuberwayCMS.showSuccess(copiedMessage)
            }
        })
    }
        ...
        $element.editable({
            ...
            success: function(response) {
                if (response.error && response.message) {
                    HuberwayCMS.showError(response.message)
                }
            },
            error: function(response) {
                HuberwayCMS.handleError(response)
            }
        });
    }
...
$(() => {
    ...
    new HuberwayCMS()
    window.HuberwayCMS = HuberwayCMS
})
